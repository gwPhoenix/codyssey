...
na908158800@c3r1s4 permission_test % ls -ld test_directory   #디렉토리 권한 확인
drwx-w--w-  3 na908158800  na908158800  96 Mar 31 17:09 test_directory   #권한 변경됨 drwx--xr-x → drwx-w--w-
```
...
na908158800@c3r1s4 docker_image_custom % cat > html/index.html << 'EOF'   #HTML 작성 시작,EOF가 추가로 나오면 마지막 부분
...
- 호스트에 디렉토리 생성 & 파일 생성
```
mkdir ~/mydata   #사용자폴더 아래 mydata 폴더 생성
echo Hello From Host > /Users/na908158800/mydata/test.txt   #"/Users/na908158800/mydata/test.txt"파일에 "Hello From Host" 기록
```
- 바인드 마운트로 컨테이너 실행
```
docker run -it -v ~/mydata:/data ubuntu bash
#우분투를 실행해서, 호스트폴더(~/mydata)와 컨테이너 폴더(/data)를 연결(=바인트 마운트)하고, 대화할 수 있도록 터미널 열기
```
- 호스트 변경 전 - 컨테이너에서 확인
```
cat /data/test.txt   #호스트 파일 확인
Hello From Host
```
- 호스트에서 파일 수정
```
echo Updated From Host > /Users/na908158800/mydata/test.txt   #"/Users/na908158800/mydata/test.txt"파일에 "Updated From Host" 덮어쓰기
```
- 호스트 변경 후 - 컨테이너에서 확인
```
cat /data/test.txt   #호스트 파일 확인
